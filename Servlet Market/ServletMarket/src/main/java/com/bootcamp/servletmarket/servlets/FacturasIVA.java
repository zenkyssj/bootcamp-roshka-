 ...
 * @author Jose
 */
@WebServlet(name = "FacturasIVA", urlPatterns = {"/FacturasIVA"})
public class FacturasIVA extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try (Connection conn = Conexion.connect(getServletContext())){
            ...
            while (rs.next()){
                ...
                row.put("total_sin_iva", rs.getInt("total_sin_iva"));
                row.put("iva_10", rs.getInt("iva_10"));
                row.put("total_con_iva", rs.getInt("total_con_iva"));
                lista.add(row);
            }

            request.setAttribute("facturas_iva", lista);

            request.getRequestDispatcher("facturasiva.jsp").forward(request, response);
